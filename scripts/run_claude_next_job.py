...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the next pending Claude experiment job.")
    ...
    job = select_next_provider_job(project_root=project_root, provider="anthropic", run_key=args.run_key)
    if job is None:
        print("No pending Claude job found, or requested run_key is already complete.")
        return

    print("Selected Claude job")
    print("===================")
